...
| `get_available_slots` | `doctor_name` (string), `limit` (number, optional) | before offering times |
...
| `check_insurance` | `provider_name` (string) | insurance verification |
...
Copy the full prompt from **`VAPI_SYSTEM_PROMPT.txt`** into your VAPI assistant system prompt.
...
- Insurance: `check_insurance` only.
...
4. Assistant calls `get_available_slots` → `2026-05-26 at 09:00; 2026-05-26 at 10:30`  
...
- **Empty slots:** slot may already be booked; re-run `python names.py` on a fresh `hospital.db`.
